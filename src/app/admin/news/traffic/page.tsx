...
export default function NewsManagement() {
  ...
  async function loadPosts() {
    const res = await fetch("/api/news?category=traffic");
          ...
          <div className="p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 sm:mb-0">
                Quản lý Tin tức giao thông
              </h1>
              <Link
                href="/admin/news/traffic/create"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {posts.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition">
                      ...
                      {/* This cell was missing, causing misalignment */}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                        <Link
                          href={`/admin/news/traffic/edit/${p.id}`}
