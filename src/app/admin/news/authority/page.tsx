...
export default function AuthorityNewsManagement() {
  const [posts, setPosts] = useState<Post[]>([]);

  async function loadPosts() {
    const res = await fetch("/api/news?category=authority");
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6">
              ...
              <Link
                href="/admin/news/authority/create"
                  ...
                  {posts.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                        <Link href={`/admin/news/authority/edit/${p.id}`} className="text-blue-600 hover:text-blue-900 inline-flex items-center"><FaEdit className="mr-1" /> Sửa</Link>
